...
const CloudinaryReceiver = require('./cloudinary');

    aws.config.update({
        secretAccessKey: '/1Je/cIom1O6tkf+IRwylrwTs3TP11sBvq/YoFDL',
        accessKeyId: 'AKIAZ55QWUVIHCONVHHG',
        region: 'us-east-1'
    });
...
function saveUser(req, res){
    ...
    if(params.name && params.email && params.password){
            ...
            user.estado   = 1;
            user.identidad= params.identidad;
            user.idescuela = null;
            //
            User.find({ $or: [
...
async function followUserIds(user_id) {
    ...
    var followed = await Follow
        .find({ followed: user_id })
...
function updateUser(req, res){
    ...
    if(userId != req.user.sub){
        return res.status(300).send({message:'no tienes permiso'})
    }
    User.find(
        { $or: [
...
// Edicion de datos de usuario
function updateUserAcademic(req, res){
    var userId= req.params.id;
...
// Subir imagen
function uploadImageAmazon(req, res){
  //  var userId = req.params.id;
   console.log('pasa por aqui..id__'+req.files);
    if(req.files){
        var file = req.files;
        var file_path = req.files.image.path;
...
function uploadCloudinary (req, res){
    ...
    function removeFilesOfUploadCloudinary(res, file_path, message){
        cloudinary.config({ 
        ...
        if (file_path != null) {
            var urlperfil='https://res.cloudinary.com/djempmk2c/image/upload/';
            urlperfil = urlperfil +file_path;
            cloudinary.v2.uploader.destroy(urlperfil, function(error,result) {
